import { getDefaultGlobal, Predicate, copyAttrs, findItr, mapValuesToArray, nextUUID, tryCatch, applyFnWithTry } from './core';
import * as Paho from './paho-mqtt';
// import * as Paho from 'paho-mqtt';

// 顶层对象
const global = getDefaultGlobal();

// 初始化的问题，Paho不被提前使用一下，这个对象就变成了 {}
// tryCatch(() => console.log('Paho', Paho ? 'exist' : Paho));
tryCatch(() => { if(!global.Paho) { global.Paho = Paho; } });

/**
 * 生成客户端ID
 */
export const nextClientId = (prefix = 'mqttjs_') => prefix + nextUUID().substring(0, 12);

/**
 * MQTT客户端
 */
export class PahoMqttClient {
  ...
  protected readonly dispatcher = <MqttMessageDispatcher>{
    ...
    match(topic, subscription): boolean {
      if (subscription.topics.size > 0) {
        ...
        if (findItr(subscription.topics.values(), (v: any) => v.topic.match(msgTopic))) {
          return true;
        }
      }
      return false;
    ...
    dispatch(client, topic, msg) {
      this.subscriptions.forEach(subscription => {
        if (this.match(topic, subscription)) { // 匹配符合订阅规则的主题
          try {
            // 分发给订阅者
    ...
    getTopics(filter: Predicate<MqttSubscription> = (ms) => true) {
      if (this.subscriptions.size <= 0) {
        return <string[]>[];
      }
      let topics = new Set<string>();
      this.subscriptions.forEach(ms => filter(ms) && ms.getTopics(mt => true).forEach(mt => topics.add(mt.topic.topicName)));
      return [...topics];
    },

    getUniqueTopics(subscription) {
      let allTopics = this.getTopics((ms) => ms != subscription);
      return subscription.getTopics(mt => true).filter(mt => !allTopics.includes(mt.topic.topicName)).map(mt => mt.topic.topicName);
    },
  };

  constructor(
    public opts: MqttOptions
  ) {
    this.opts = copyAttrs(opts, <MqttOptions>{ clientId: nextClientId(), port: 8083, path: '/mqtt', userName: '', password: '' });
    tryCatch(() => {
      this.raw = new global.Paho.Client(opts.host, opts.port, opts.path, this.clientId);
  ...
  protected rawSubscribe(ms: MqttSubscription, topic: string, qos: number = 0) {
    if (this.isConnected()) {
  ...
  connect() {
    ...
    this.raw.connect(copyAttrs({ userName, password }, <any>{
      onSuccess: (res: any) => this.onConnect(res),
  ...
  subscribe(subscriber: MqttSubscriber, topic: string, qos: number = 0) {
    ...
    if (!subscription) {
      subscription = <MqttSubscription>{
        ...
        getTopics(filter) {
          return mapValuesToArray(this.topics).filter(filter);
        },
        hasTopic(topic) {
    ...
    if (!subscription.hasTopic(topic)) {
      ...
      if (this.isConnected()) {
        ...
        tryCatch(() => this.rawSubscribe(subscription, topic, qos));
      }
    }
  }
   ...
   * 连接成功
   */
  protected onConnect(res: any) {
    try {
      this.stopAutoReconnect();
      // 订阅
      this.dispatcher.subscriptions.forEach(ms => {
        tryCatch(() => ms.topics.forEach(mt => this.rawSubscribe(ms, mt.topic.topicName, mt.qos)));
        tryCatch(() => applyFnWithTry(ms.subscriber.onConnected, this));
      });
    } catch (err) {
  ...
  protected onConnectionLost(res: ConnectLost) {
    try {
      if (res.errorCode !== 0) {
        ...
        // 客户端可能自动断开了，这时需要重连
        this.dispatcher.subscriptions.forEach((ms, topic) => applyFnWithTry(ms.subscriber.onConnectLost, this, res));
      } else {
        this.stopAutoReconnect(); // 停止自动重连
        // 断开连接
        this.dispatcher.subscriptions.forEach((ms, topic) => applyFnWithTry(ms.subscriber.onDisconnected, this));
      }
    } catch (err) {
  ...
  protected onMessageDelivered(message: MqttMessage) {
    this.dispatcher.subscriptions.forEach((ms, topic) => applyFnWithTry(ms.subscriber.onMessageDelivered, this, message));
  }
...
export interface MqttMessageDispatcher {
   ...
   * @param msg 消息
   */
  dispatch(client: PahoMqttClient, topic: string, msg: MqttMessage): void;
   ...
   * @returns 获取匹配的主题
   */
  getTopics(filter: Predicate<MqttSubscription>): string[];
...
export interface MqttSubscription {
   ...
   * @param filter 过滤器
   */
  getTopics(filter: Predicate<MqttSubscriptionTopic>): MqttSubscriptionTopic[];
...
export interface MqttSubscriber {
   ...
   * @param msg 消息
   */
  onMessage(client: PahoMqttClient, topic: string, msg: MqttMessage): void;
   ...
   * @param client 客户端
   */
  onConnected?(client: PahoMqttClient): void;
   ...
   * @param client 客户端
   */
  onDisconnected?(client: PahoMqttClient): void;
   ...
   * @param client 客户端
   */
  onConnectLost?(client: PahoMqttClient, lost: ConnectLost): void;
   ...
   * @param client 客户端
   */
  onMessageDelivered?(client: PahoMqttClient, msg: MqttMessage): void;
 ...
 * 默认的 topic 分割器
 */
export const TOPIC_SLICER = <Slicer>((b, position, ch) => ch == '/')
